    ...
    def validate_api_key(self, provider: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if provider == "openai":
                ...
                llm = ChatGoogleGenAI(model="gemini-2.0-flash", api_key=api_key)
...
def update_provider():
    ...
    try:
        ...
        if provider not in ["openai", "anthropic", "qwen", "google"]:
            return jsonify({"error": "Provider must be 'openai', 'anthropic', 'qwen', or 'google'"})
        ...
        if provider == "anthropic":
            ...
            if not analyzer.config["agent_llm_model"].startswith("gemini"):
                analyzer.config["agent_llm_model"] = "gemini-2.0-flash"
            if not analyzer.config["graph_llm_model"].startswith("gemini"):
                analyzer.config["graph_llm_model"] = "gemini-2.0-flash"
        else:
