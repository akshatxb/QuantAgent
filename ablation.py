...
BULLISH_KW = [
    "bullish", "upward", "uptrend", "above", "oversold", "positive",
    "buy", "long", "support", "bounce", "rising", "higher", "ascending",
    "crossover above", "golden cross",
]
BEARISH_KW = [
    "bearish", "downward", "downtrend", "below", "overbought", "negative",
    "sell", "short", "resistance", "breakdown", "falling", "lower", "descending",
    "crossover below", "death cross",
]


def _indicator_only_decision(indicator_report: str) -> str:
    """
    Simple keyword-majority heuristic over the indicator report text.
    Returns "LONG" or "SHORT" (never HOLD — this is the no-LLM baseline).
    """
    text  = indicator_report.lower()
...
def _stats(signals: list[str], outcomes: list[str], pnl_mags: list[float]) -> dict:
    ...
    active_sigs    = [(s, o, m) for s, o, m in zip(signals, outcomes, pnl_mags) if s != "HOLD"]
    hold_sigs      = [(s, o, m) for s, o, m in zip(signals, outcomes, pnl_mags) if s == "HOLD"]
    ...
    pnl_arr = np.array([
        m if s == o else -m
        for s, o, m in active_sigs
    ]) if active_sigs else np.array([0.0])

    tp = sum(s == "LONG"  and o == "LONG"  for s, o in zip(active_signals_list, active_outcomes))
    tn = sum(s == "SHORT" and o == "SHORT" for s, o in zip(active_signals_list, active_outcomes))
    fp = sum(s == "LONG"  and o == "SHORT" for s, o in zip(active_signals_list, active_outcomes))
    fn = sum(s == "SHORT" and o == "LONG"  for s, o in zip(active_signals_list, active_outcomes))
    ...
    long_sigs  = [(s, o) for s, o in zip(active_signals_list, active_outcomes) if s == "LONG"]
    short_sigs = [(s, o) for s, o in zip(active_signals_list, active_outcomes) if s == "SHORT"]
    long_acc   = sum(s == o for s, o in long_sigs)  / len(long_sigs)  if long_sigs  else 0.0
    short_acc  = sum(s == o for s, o in short_sigs) / len(short_sigs) if short_sigs else 0.0
...
def run_ablation(
    ...
    # ------------------------------------------------------------------
    ind_signals = [
        _indicator_only_decision(r.get("indicator_report", ""))
        for r in records
    ]
    ...
    pnl_mags_dir       = [r.get("pnl_magnitude", abs(r.get("pnl", 0.01))) for r in directional_records]

    v3 = _stats(agent_signals_dir, outcomes_dir, pnl_mags_dir)
    v3["variant"] = "full_quantagent"
    v3["label"]   = "Full QuantAgent"
    v3["description"] = "Complete pipeline: indicator + pattern + trend agents → LLM decision (directional calls only)"
    ...
    if prediction_layer is not None and prediction_layer.trained:
        ...
        flips = sum(
            p != a for p, a in zip(pred_signals, all_agent_signals)
        )
    ...
    _log.ok(f"random={v1['accuracy']:.1%}  indicators={v2['accuracy']:.1%}  agent={v3['accuracy']:.1%}" + (f"  +pred={v4['accuracy']:.1%}" if v4 else ""))
    ...
    for path in sorted(ABLATION_DIR.glob("ablation_*.json"), reverse=True):
        try:
            with open(path) as f:
                data = json.load(f)
            m   = data.get("meta", {})
            v3  = next((v for v in data.get("variants", []) if v["variant"] == "full_quantagent"), {})
            v1  = next((v for v in data.get("variants", []) if v["variant"] == "random_baseline"), {})
            results.append({
                "filename":        path.name,
                "ticker":          m.get("ticker"),
                "market":          m.get("market"),
